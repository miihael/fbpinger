...
struct libicmp {
    ...
    struct sockaddr *addr;        /* packed host IP */
    struct timeval  tv;         /* Packet send time */
    long            triptime;   /* Round trip time */
    struct fbr_context *fctx;   /* fiber context */
};

typedef struct libicmp libicmp_t;

int         icmp_resolve(libicmp_t *isock);
char       *icmp_ntoa(struct addrinfo *addr, char *buf, size_t len);

libicmp_t  *icmp_open(libicmp_t *isock, int socket_fd, char *host, unsigned short id, unsigned ttl);
//size_t      icmp_recv(libicmp_t *isock, char *buf, u_int8_t type, int timeout);
size_t      icmp_recv_one(FBR_P_ u_int8_t type, int socket_fd, char *buf, libicmp_t *isock);
int         icmp_send(FBR_P_ libicmp_t *isock, u_int8_t type, char *payload, size_t len);
